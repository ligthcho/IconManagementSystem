    ...
    var methods = {
        ...
        createTable: function () {
            ...
            for (i = 0; i < options.colHeaders.length; i++) {
                ...
                colID = options.colID[i];
                // Column type hidden
                if (options.columns[i].type == 'hidden') {
                    // TODO: when it is first check the whole selection not include
                	tr += '<td id="col-' + i + '" style="display:none;" title="' + header + '" data-header="' + colID + '">' + header + '</td>';
                } else {
                    // Other column types
            ...
            if (!$('.jexcel_corner').length) {
                ...
                // Global mouse click down controles//锟节憋拷锟絫d锟叫帮拷锟斤拷锟斤拷
                    ...
                    if (e.target.id == 'corner') {
                        ...
                        if ($(table).is('.jexcel')) {
                            ...
                            if ($(e.target).parent().parent().is('tbody')) {
                                // Update row label selection锟斤拷锟斤拷锟斤拷选锟斤拷锟叫的ｏ拷
                                $(".jexcel_label").removeClass('selected-row');
                                $('.jexcel_label').parent().find("td").removeAttr("style");
                                ////锟斤拷锟斤拷锟斤拷锟絫d时锟斤拷锟斤拷选锟斤拷锟斤拷锟叫革拷值锟斤拷锟皆讹拷锟斤拷谋锟斤拷锟?
                                selectedRowDel = $(e.target);
                                if ($(e.target).is('.jexcel_label')) {
                                    //锟斤拷选锟叫的碉拷前锟叫加憋拷志锟斤拷class                                    
                                    $(e.target).addClass('selected-row');
                                    $('.selected-row').parent().find("td").css("background-color","#eee");
                ...
                $(document).on('mousemove', function (e) {
                    if ($.fn.jexcel.current) {
                        ...
                        if ($.fn.jexcel.defaults[$.fn.jexcel.current].rowDrag == true) {
                            if ($(e.target).parent().parent().is('tbody')) {
                                // Update row label selection(锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷拽)
                                if ($(e.target).is('.jexcel_label')) {
                                    ////锟斤拷拽时为锟斤拷拽锟斤拷锟斤拷元锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷删锟斤拷
                                    ////$(e.target).addClass("selected-row");
                                    if ($(e.target).outerWidth() - e.offsetX < 8) {
                ...
                $("#add-row").on("click", function () {
                	console.log($.fn.jexcel.current);
                	if ($.fn.jexcel.current == undefined) {
                		alert("请选择行");
                		return;
                	}
                    $.fn.jexcel.current = $.fn.jexcel.current || 'my';
                ...
                $("#delete-rows").on("click", function () {
                    ...
                    if (!$($.fn.jexcel.selectedCell).hasClass('edition')) {
                        ////锟皆硷拷锟斤拷态写锟斤拷锟斤拷去锟斤拷值
                        $.fn.jexcel.current = $.fn.jexcel.current || 'my';
                        if ($('.jexcel_label').hasClass("selected-row")) {
                            if (selectedRowDel) {
                                if (confirm('delete rows?')) {
                                    var rows = $('#' + $.fn.jexcel.current).find('tbody').find('.jexcel_label.selected-row');
                ...
                //鎸夐敭鎺у埗鍔熻兘  enter == 13閿坊鍔犱笅涓€琛岋紝tab==9閿坊鍔犱竴锟? delete==46閿垹闄ゅ綋鍓嶅崟鍏冩牸鍐呭
                    ...
                    if ($.fn.jexcel.current) {
                        ...
                        if ($.fn.jexcel.selectedCell) {
                            ...
                            if (e.which == 37) {
                                ...
                                e.preventDefault();
                            } else if (e.which == 46) {    //delete锟斤拷删锟斤拷锟叫伙拷锟斤拷锟斤拷锟斤拷
                                // Delete (erase cell in case no edition is running)
                                if ($.fn.jexcel.defaults[$.fn.jexcel.current].editable == true) {
                                    if (!$($.fn.jexcel.selectedCell).hasClass('edition')) {
                                        if ($.fn.jexcel.selectedRow) {
        ...
        updateSelection: function (o, d, ignoreEvents, origin) {
            ...
            //$(cells).removeClass('selected-row');
            ...
            if (o && d) {
                ...
                for (i = px; i <= ux; i++) {
                    for (j = py; j <= uy; j++) {
                        ...
                        //$(main).find('#row-' + i).addClass('selected-row');
                        $(main).find('#row-' + j).addClass('selected');
                        $(main).find('#row-' + j).addClass('selected-row');   //锟斤拷拽时也锟侥憋拷选锟斤拷锟斤拷锟絚lass
                        $('.selected-row').parent().find("td").css("background-color", "#eee");  //锟斤拷锟斤拷色锟斤拷枚锟斤拷锟缴?
                    }
                }
