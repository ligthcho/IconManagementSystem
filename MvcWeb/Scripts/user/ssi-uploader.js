...
(function (root, factory) {
	...
	var Ssi_upload = function (element, options) {
        ...
        this.definedFunction = this.options.definedFunction;
        this.init(element);
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        if (!this.options.preview) {
        	...
        	if (this.options.tag == 1) {
        		$uploadBox = $('<div id="ssi-previewBox" class="ssi-uploadBox ssi-previewBox ' + (this.options.dropZone ? 'ssi-dropZonePreview ssi-dropZone"><div id="ssi-DropZoneBack">' + this.language.drag + '</div>' : '">') + '</div><div class="clearfix icon-previewBox"><div class="icon-up-wrapper clearfix"></div><div class="icon-info" style="display:none;">\
                            <ul>\
                                <li>图片尺寸：<span class="icon-size" id="picSize"></span></li>\
                                <li>分辨率：<select class="icon-dpi" id="picResolution"><option selected="selected" value="0">96dpi</option> <option value="1">72dpi</option></select></li>\
                                <li>图片名称：<input class="icon-name" type="text" name="" value="上传时的名称" readonly="true"></li>\
                                <li>标签：<input class="icon-tag"  id="img-tag" type="text" name="" placeholder="以|分隔"></li>\
                                <li class="ssi-treeBox" id="img-proid">\
                                    所属系统：\
                                    <div class="project-tree">\
							        <!-- 这里树结构<ul></ul> -->\
								       <div class="folder_tree_json" id="sys_tree">\
                                       </div>\
                                   </div>\
								</li>\
                            </ul>\
                        </div><div class="upload-btn" style="display:none;">\
                               <input type="button" class="uploadfilesBtn" id="submitfilesBtn"  name="" value="上传">\
                             </div></div>');
        		this.$element.append($uploadBox);
        	} else {
        		$uploadBox = $('<div id="ssi-previewBox" class="ssi-uploadBox ssi-previewBox ' + (this.options.dropZone ? 'ssi-dropZonePreview ssi-dropZone"><div id="ssi-DropZoneBack">' + this.language.drag + '</div>' : '">') + '</div><div class="clearfix icon-previewBox" style="display: block;"><div class="icon-up-wrapper clearfix"></div><div class="icon-info" style="display:block;">\
                            <ul>\
                                <li>图片尺寸：<span class="icon-size" id="picSize"></span></li>\
                                <li>分辨率：<select class="icon-dpi" id="picResolution"><option selected="selected" value="0">96dpi</option> <option value="1">72dpi</option></select></li>\
                                <li>图片名称：<input class="icon-name" id="picName"  type="text" name="" value="上传时的名称"></li>\
                                <li>标签：<input class="icon-tag"  id="img-tag" type="text" name="" placeholder="以|分隔"></li>\
                                <li class="ssi-treeBox" id="img-proid">\
                                    所属系统：\
                                    <div class="project-tree">\
                        ...
                        </div><div class="upload-btn" style="display:block;">\
                               <input type="button" class="uploadfilesBtn" id="submitfilesBtn"  name="" value="修改">\
                             </div></div>');
        		this.$element.append($uploadBox);
        		this.options.definedFunction();//修改页赋值
        	}
        }
        var thisS = this;
        //点击上传按钮。触发input点击
        ...
        $(".uploader-file-click").click(function () {
            $input.trigger('click');
        });
        // 绑定上传改变事件(点击文件确定后触发的事件)
        $input.on('change', function () { //choose files
            thisS.toUploadFiles(this.files);
            $input.val('');
          //  $("#ssi-previewBox").remove();
            // 把之前的背景隐藏
        ...
        $uploadBox.on('click', '.ssi-removeBtn', function (e) { //remove the file from list
            ...
            if (thisS.pending === 0) {
                ...
                // 如果剩下最后一个图标点删除则还原上传页面
                $(".up-file > a").css("display","inline-block");
                $(".up-file > .up-tips").css("display","block");
                $(".ssi-uploader .icon-info").css("display","none");
                $(".ssi-uploader .up-detail-tips").css("display","none");
                $(".ssi-uploader .upload-btn").css("display","none");
                $(".icon-previewBox").css("display","none");

            }
            if (thisS.options.preview) { //if preview is true
                // 删除该图标的详细信息的图片名称
                $currentTarget.parents(".icon-previewBox").find(".icon-info .icon-name").val(" ");
...
//----------------------------UPLOADFILES------------------------------------
        $uploadBtn.click(function () {// upload the files
        	thisS.uploadFiles();//setAgs() 上传按钮点击事件
        });
...
// //////////////////////////////////////////
        function setupReader() {
            ...
            if (thisS.options.preview) {
                var getTemplate = function (content) {
                	return '<table class="ssi-imgToUploadTable ssi-pending" data-info='+JSON.stringify(dataInfo)+'>' +
                     '<tr><td class="ssi-upImgTd">' + content + '</td></tr>' +
                     '<tr><td><div id="ssi-uploadProgress' + index + '" class="ssi-hidden ssi-uploadProgress"></div></td></tr>' +
                     '<tr class="del-icon"><td><button data-delete="' + index + '" class=" ssi-button error ssi-removeBtn"><span class="glyphicon glyphicon-trash"></span></button></td></tr>' +
                     '<tr class="icon-current-name"><td class="icon-cur-name">' + cutFileName(filename, ext, 15) + '</td><td class="icon-cur-size"></td><td  class="icon-cur-dpi"></td></tr></table>'
                };
 ...
 //////////////////////// 添加table到页面中，也就是添加图标
                            ...
                            // ////////////////
                        	$(".icon-up-wrapper").append(imgContent);
                            // /////////////////////
                            $uploadBtn.prop("disabled", false);
                            $clearBtn.prop("disabled", false);
                            setTimeout(function () {
                    ...
                    // //////////////////////////////////
                    $(".icon-up-wrapper").append(getTemplate('<div class="document-item" href="test.mov" filetype="' + ext + '"><span class = "fileCorner"></span></div>'));
                    //////////////////////////////////////
                    // $(".table-content-wrap").append(getTemplate('<div class="document-item" href="test.mov" filetype="' + ext + '"><span class = "fileCorner"></span></div>'));
                    j++;
                }
            ...
            var setImg = function () {//load the images
                for (var i = 0; i < imgs.length; i++) {
                    if (imgs[i] !== null) {
                    	// ////////////
                        $(".icon-up-wrapper").find("#ssi-uploadProgress" + i).parents('table.ssi-imgToUploadTable')
    ...
    Ssi_upload.prototype.uploadFiles = function (args) {// upload the pending files
        if (this.pending > 0) {
            ...
            var ind = thisS.toUpload[i].index,
				dinfo = "",
                choose="";
            thisS.$element.find(".ssi-imgToUploadTable").each(function () {  //循环图标table 拿到第一个图标的data-info
            	dinfo = JSON.parse($(this).attr("data-info"));
            	if (ind == dinfo.index) {
            		choose = dinfo;
            	}
            });

            formData.append('files[]', thisS.toUpload[i]);//append the first file to the form data 上传第一个文件的data(上传第2。3个文件...以后的data数据，在下面)
            $.each(choose, function (key, value) {// append all extra data
        ...
        function ajaxLoopRequest(formData, ii) {
            ...
            if (i < thisS.toUpload.length) {// if more files exist start the next request 上传第2.3个文件...以后的data数据，在这里
            	formData = new FormData();
            	var ind = thisS.toUpload[i].index,
		            sinfo = "",
            	    schoose = "";
            	thisS.$element.find(".ssi-imgToUploadTable").each(function () {  //循环图标table 拿到第一个之后图标的data-info
            		sinfo = JSON.parse($(this).attr("data-info"));
            		if (ind == sinfo.index) {
            			schoose = sinfo;
            		}
            	});
            	$.each(schoose, function (key, value) {
            	    formData.append(key, value);
            	}); 
                //$.each(thisS.options.data, function (key, value) {
                //    formData.append(key, value);
                //});   原来的代码
                console.log(thisS.toUpload[i]);
