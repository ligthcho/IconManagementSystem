...
namespace Logic
{
	...
	public class PageLogic
	{
		...
		*/
		...
		public static void PageOut(System.Web.UI.Page page,string strOutput)
		{
			try
			{
				#region 记录相应日志
		...
		/// <param name="strOutput"></param>
		public static void PageOut(HttpContext httpContext,string strOutput)
		{
			try
			{
				#region 记录相应日志
				if(PAGE_LOG_WRITE_LOG)
				{
					...
					string strLog = DateTime.Now.ToString("HH:mm:ss.fff") + "\t" + ClientModel.GetIPAdderss().PadLeft(15,' ') + "\t" + httpContext.Request.RawUrl + strForm;//+"\t"+strOutput.Replace("\r",string.Empty).Replace("\n",string.Empty).Replace("\t",string.Empty);
					LogLogic.Write(strLog,PAGE_LOG_WRITE_LOG_PATH,PAGE_LOG_WRITE_LOG_EXTENSION);
				}
				#endregion 记录相应日志

				httpContext.Response.Clear();
				httpContext.Response.ContentType = PAGE_DATA_TYPE;
				httpContext.Response.Charset = PAGE_DATA_CHARSET;//设置输出流的字符集-中文
				httpContext.Response.ContentEncoding = System.Text.Encoding.GetEncoding(PAGE_DATA_CHARSET);//设置输出流的字符集
				httpContext.Response.Write(strOutput);
				HttpContext.Current.ApplicationInstance.CompleteRequest();
			}
			catch(Exception ex)
			{
